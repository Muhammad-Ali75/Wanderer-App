...
class Home extends Component {
  ...
  Header = () => {
    return (
      <View style={{ ...styles.header }}>
        ...
        />
        <Text
          style={{ fontSize: EStyleSheet.value("16rem"), fontWeight: "600" }}
        >
          Shopping
        </Text>
        <FontAwesome
  ...
  Product = ({ item, index }) => {
    ...
    const [addToCartContHeight, setAddToCartContHeight] = useState(
      new Animated.Value(EStyleSheet.value("30rem"))
    );
            ...
            {this.state.categories.map((item, index) => {
              return (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    this.setState({ selectedCatogoryIndex: index })
                  }
                >
                  <Text
                    style={{
                      fontSize: EStyleSheet.value("14rem"),
                      fontWeight:
                        this.state.selectedCatogoryIndex == index
                          ? "600"
                          : "400",
                      color:
                        this.state.selectedCatogoryIndex == index
